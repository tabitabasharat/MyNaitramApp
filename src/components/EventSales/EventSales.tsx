...
function createData(name: number, calories: string, fat: number, carbs: number) {
...
const rowsticket = [tickets(1, "Premium", 100, 99), tickets(2, "Gold", 80, 78), tickets(3, "Silver", 150, 120)];
  ...
  return (
    <div className="pt-[120px] pb-[59.12px] lg:pb-[26.25px] px-[24px] event-bg-effect lg:px-[100px] xl:px-[216px] md:pt-[132px] mx-auto">
      ...
      <div className="gap-[32px] mb-[24px] lg:mb-[30px] flex lg:flex-row flex-col lg:gap-[42px]">
        ...
        <div>
          <div className="bg-[#00A849] border border-solid border-[#757575] py-[8px] px-[12px] w-[135px] lg:w-[135px] rounded-[100px]">
            <p className="text-[#030303] italic text-[11px] lg:font-extrabold font-[900] lg:text-[12px]">FEATURED EVENT</p>
          </div>
          <p className="font-extrabold text-[32px] lg:text-[48px] mb-[12px] lg:mb-[24px] mt-[12px]">{eventSales?.salesData?.data?.event?.name}</p>
          <div className="flex items-center">
            <div className=" w-[40px] h-[40px] rounded-[8px]">
              <Image
                src={eventSales?.salesData?.data?.event?.user?.profilePicture ? eventSales?.salesData?.data?.event?.user?.profilePicture : id}
                width={40}
                height={40}
                sizes="40px"
            ...
            <div>
              <p className="ps-[8px] pe-[4px] text-sm lg:font-bold font-[900]">{eventSales?.salesData?.data?.event?.user?.fullname}</p>
            </div>
          </div>
          <div className="mt-[16px] lg:mt-[24px]">
            <p className="flex items-center text-base font-bold mb-[12px]]">
              {" "}
              <Image src={location} className="pe-[8px] w-[30px] h-[30px]" alt="location" />
              {eventSales?.salesData?.data?.event?.location}
            </p>
            <p className="flex items-center text-base font-bold mb-[12px]]">
              {" "}
              <Image src={clander} alt="clander" className="pe-[8px] w-[30px] h-[30px]" />
              {ConvertDate(eventSales?.salesData?.data?.event?.startTime)}
            </p>
            <p className="flex items-center text-base font-bold mb-[12px]]">
              {" "}
              <Image src={time} alt="time" className="pe-[8px] w-[30px] h-[30px]" />
              {ConvertTime(eventSales?.salesData?.data?.event?.startTime)} - {ConvertTime(eventSales?.salesData?.data?.event?.endTime)}{" "}
            </p>
          </div>
        </div>
      </div>
        ...
        <TableContainer component={Paper} sx={{ boxShadow: "none" }}>
          ...
          >
            ...
            <TableBody className="border-0">
              {eventSales?.salesData?.data?.ticketTypes?.map((rows: any, index: any) => (
                <TableRow
                  key={rows.type}
                  sx={{
        ...
        <div className="flex items-center mb-[17px] lg:mb-[24px] justify-between">
          <div>
            <p className="lg:text-[14px] text-[#D9D9D9] font-normal">
              Scanned Tickets
              <br />
              <span className="text-[#00D059] text-[32px] font-bold pt-[2px]">{eventSales?.salesData?.data?.totaChecked}</span>
            </p>
          </div>
          <div>
      ...
      <div className=" mb-[32px] gradient-slate border rounded-lg border-muted mt-[32px] px-[16px] lg:px-[24px] pb-[26px] lg:pb-[24px] pt-[16px] lg:pt-[26px]">
        <div className="flex items-center mb-[17px] lg:mb-[24px] justify-between">
          <div>
            <p className="lg:text-[32px] text-[24px] font-normal lg:font-bold">Scanned Ticket Categories</p>
          </div>
          <div>
            <Image src={filter} alt="img" />
          </div>
